...
export const useEmbodee = () => {
    ...
    const initialize = useCallback(async (isRetry = false) => {
        if (initializationRef.current && !isRetry)
            return;
        if (!isRetry) {
     ...
     */
    const waitForEmbodeeLoaderWithRetry = () => {
        return new Promise((resolve, reject) => {
            const maxRetries = 150; // 150 * 200ms = 30 seconds timeout
            let attempt = 0;
            const interval = setInterval(() => {
                if (typeof window !== 'undefined' &&
                    typeof window.EmbodeeLoader?.init === 'function') {
     ...
     */
    const fetchProductDataWithRetry = async (urlParams) => {
        try {
     ...
     */
    const initializeViewerWithRetry = async (urlParams, data) => {
        try {
     ...
     */
    const shouldRetryInitialization = (error, currentRetryCount) => {
     ...
     */
    const initializeViewer = async (urlParams, _data) => {
        const viewerConfig = await window.EmbodeeLoader.init({
     ...
     */
    const setupViewerEvents = (viewer) => {
        ...
        const handleProductReady = () => {
            try {
                ...
                logger.info('CONFIGURATOR', 'Product ready - UI structure parsed successfully', {
                    ...
                    optionTypes: parsedOptions.map(opt => opt.type || 'unknown')
                }, 'useEmbodee');
            }
            catch (err) {
     ...
     */
    const setValue = useCallback(async (componentCode, property, value, isCustom = false) => {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                // Don't retry on certain error types
                if (err instanceof Error && (err.message.includes('Invalid component') ||
     ...
     */
    const getValue = useCallback((componentCode, property) => {
        const viewer = configuratorRef.current;
        if (!viewer || !isInitialized) {
            return '';
        }
